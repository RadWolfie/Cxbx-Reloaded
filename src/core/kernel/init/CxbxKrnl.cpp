...
FILE* CxbxrKrnlSetupVerboseLog(int BootFlags)
{
	std::string tempStr;
	// Get KernelDebugMode :
	CxbxKrnl_DebugMode = DebugMode::DM_NONE;
	if (cli_config::GetValue(cli_config::debug_mode, &tempStr)) {
		CxbxKrnl_DebugMode = (DebugMode)std::atoi(tempStr.c_str());
	}

	// Get KernelDebugFileName :
	CxbxKrnl_DebugFileName = "";
	if (!cli_config::GetValue(cli_config::debug_file, &CxbxKrnl_DebugFileName)) {
		CxbxKrnl_DebugFileName = "";
	}

	// debug console allocation (if configured)
	if (CxbxKrnl_DebugMode == DM_CONSOLE)
	{
		if (AllocConsole())
		{
	...
	{
		FreeConsole();
		if (CxbxKrnl_DebugMode == DM_FILE) {
			// Peform clean write to kernel log for first boot. Unless multi-xbe boot occur then perform append to existing log.
			FILE* krnlLog = freopen(CxbxKrnl_DebugFileName.c_str(), ((BootFlags == DebugMode::DM_NONE) ? "wt" : "at"), stdout);
